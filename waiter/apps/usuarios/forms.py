...
class UserCreate(UserCreationForm):
	...
	username = forms.RegexField (widget=forms.TextInput(attrs={'class':'form-control'}), label = "Nombre de usuario", max_length = 30, regex = r"^[\w'\.\-\_]+$", help_text = "Menor a 30 caracteres. Permitido: letras, dígitos y -._", error_message = "Contiene caracteres no permitidos." ,validators=[validarUsuario])
	email = forms.EmailField(widget=forms.TextInput(attrs={'class':'form-control', 'placeholder':'tucuenta@ejemplo.com'}),label='Correo electrónico', required=True, error_messages={'invalid': 'Ingrese una dirección de correo válida.'}, validators=[validarEmail])
	# Style CSS + Bootstrap
